...
const EditYTModal = ({
  ...
  const handleEditVideo = async (values) => {
    ...
    try {
      const response = await axios.post(`/api/admin/yt_video/${item._id}`, {
        yturl,
        date,
        title,
        shorts,
      ...
      if (response.data.success) {
        ...
        setSuccessModal(true);
        setError(true);
        setMsg(response.data.message || "Failed to update video.");
      }
    } catch (error) {
  ...
  const closeModal = () => {
    ...
    if (cancelTokenSource) {
      cancelTokenSource.cancel("Edit canceled by user");
            ...
            <form>
              <fieldset disabled={btnLoading ? true : false}>
                ...
                {/* <div className="category-checks grid gap-3 md:grid-cols-4 sm:grid-cols-2 grid-cols-1 mb-3">
                  {[{ label: "Shorts", value: "shorts" }].map((category, index) => (
                    <InputChecks
                      type="checkbox"
                      key={index}
                      name="shorts"
