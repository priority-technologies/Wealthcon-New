...
const NotesPage: React.FC<NotesPageProps> = ({ onLogout, onNoteSelect, onNavigate }) => {
    const [viewMode, setViewMode] = useState<ViewMode>('grid');
    const [searchTerm, setSearchTerm] = useState('');
    const [sortOrder, setSortOrder] = useState('latest');
    const [categoryFilter, setCategoryFilter] = useState('all');

    const categories = useMemo(() => ['all', ...Array.from(new Set(notesData.map(n => n.category)))], []);
    ...
    return (
        <div className="bg-[#050a14] min-h-screen text-white">
            <Navbar onLogout={onLogout} onNavigate={onNavigate} currentPage="notes" />
           
            <main className="py-8 pt-24">
                <div className="container mx-auto px-4 md:px-12">
                    <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
                       <h1 className="text-3xl md:text-4xl font-black">All Notes</h1>
                ...
                <div className="container mx-auto px-4 md:px-12">
                    {filteredAndSortedNotes.length > 0 ? (
                        viewMode === 'grid' ? (
                         ...
                         <p className="text-gray-400 text-center py-12 text-lg">No notes found. Try adjusting your search or filters.</p>
                    )}
                </div>
            </main>
        </div>
